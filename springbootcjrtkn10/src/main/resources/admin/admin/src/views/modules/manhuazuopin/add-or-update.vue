<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="作品编号" prop="zuopinbianhao" >
					<el-input v-model="ruleForm.zuopinbianhao" placeholder="作品编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.zuopinbianhao" label="作品编号" prop="zuopinbianhao" >
					<el-input v-model="ruleForm.zuopinbianhao" placeholder="作品编号" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.zuopinjieshao" label="作品介绍" prop="zuopinjieshao" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.zuopinjieshao"></span>
			</el-form-item>
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.zuopinjieshao = this.ruleForm.zuopinjieshao.replace(reg,'../../../springbootcjrtkn10/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `manhuazuopin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.manhuazuopinCrossAddOrUpdateFlag = false;
											this.parent.search();
