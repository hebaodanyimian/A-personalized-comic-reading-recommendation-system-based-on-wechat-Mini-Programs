			...
			<view :style='{"width":"100%","padding":"0 20rpx 48rpx","position":"relative","background":"#f3f5f9","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"calc(100% - 90rpx)","padding":"20rpx 0","background":"none","display":"flex","height":"auto"}'>
					<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' class="search-form round">
						...
						<input :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"0px","background":"#ffffff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.chaptertitle" type="text" placeholder="章节标题" ></input>
					</view>
					...
					<view class="uni-product-list" :style='{"padding":"0","margin":"0","flexWrap":"wrap","flex":"1","display":"flex","width":"calc(100% - 220rpx)","justifyContent":"space-between","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"20rpx 10rpx","margin":"0 0 40rpx","backgroundColor":"#fff","borderColor":"#1ccb96","borderRadius":"0","flexWrap":"wrap","borderWidth":"8rpx 0 0 0","display":"flex","width":"48%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view :style='{"width":"100%","padding":"8rpx 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('bookmarkmanhuaxiaoshuo','修改')) || (!userid && isAuthFront('bookmarkmanhuaxiaoshuo','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('bookmarkmanhuaxiaoshuo','删除')) || (!userid && isAuthFront('bookmarkmanhuaxiaoshuo','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 8rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('bookmarkmanhuaxiaoshuo','修改')) || (!userid && isAuthFront('bookmarkmanhuaxiaoshuo','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#03b882","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#03b882","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('bookmarkmanhuaxiaoshuo','删除')) || (!userid && isAuthFront('bookmarkmanhuaxiaoshuo','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#f00","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#f00","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('bookmarkmanhuaxiaoshuo','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","color":"rgb(255, 255, 255)","bottom":"80rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#EF7061","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('bookmarkmanhuaxiaoshuo','新增')" class="add-btn" @click="onAddTap()">新增</button>
			</view>
		</view>
	</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`bookmarkmanhuaxiaoshuo`, params);
                } else {
                    res = await this.$api.list(`bookmarkmanhuaxiaoshuo`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('bookmarkmanhuaxiaoshuo', JSON.stringify([id]));
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`bookmarkmanhuaxiaoshuo`, searchForm);
                } else {
                    res = await this.$api.list(`bookmarkmanhuaxiaoshuo`, searchForm);
                }
				// 如果是第一页数据置空
