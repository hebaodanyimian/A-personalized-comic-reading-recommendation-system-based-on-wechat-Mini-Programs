	...
	<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"48rpx 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"10rpx 20rpx","margin":"0","borderColor":"#1ccb96","borderRadius":"0","borderWidth":"2rpx 0","background":"#fff","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<xia-editor ref="content" :style='{"minHeight":"300rpx","border":"0px solid #efefef","width":"100%","fontSize":"30rpx","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussmanhuaxiaoshuo`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussmanhuaxiaoshuo`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussmanhuaxiaoshuo`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussmanhuaxiaoshuo`, this.ruleForm);
						}
						uni.setStorageSync('discussmanhuaxiaoshuoCleanType',true);
