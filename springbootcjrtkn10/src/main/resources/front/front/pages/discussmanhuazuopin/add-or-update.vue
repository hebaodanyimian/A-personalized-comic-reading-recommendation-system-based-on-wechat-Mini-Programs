	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussmanhuazuopin`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussmanhuazuopin`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussmanhuazuopin`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussmanhuazuopin`, this.ruleForm);
						}
						uni.setStorageSync('discussmanhuazuopinCleanType',true);
