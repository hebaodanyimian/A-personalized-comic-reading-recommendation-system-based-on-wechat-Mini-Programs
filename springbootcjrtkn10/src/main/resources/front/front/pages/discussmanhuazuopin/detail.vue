		...
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#f8f8f8","height":"auto"}'>
			<view :style='{"alignContent":"flex-start","padding":"24rpx","margin":"0 auto","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20241110/41bb20fa3b804b2191d135a6f8653ad5.png) no-repeat center top / 100% auto","display":"flex","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"40rpx auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"20"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"#03b882","borderRadius":"8rpx","background":"#c8dbd5","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="userid&&isAuth('discussmanhuazuopin','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"#03b882","borderRadius":"8rpx","background":"#c8dbd5","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('discussmanhuazuopin','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussmanhuazuopinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussmanhuazuopinCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discussmanhuazuopin', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discussmanhuazuopin');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discussmanhuazuopin');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discussmanhuazuopin', this.id);
				let reg=new RegExp('http://localhost:8080/springbootcjrtkn10/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discussmanhuazuopin',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscussmanhuazuopin', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscussmanhuazuopin',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscussmanhuazuopin/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscussmanhuazuopin',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discussmanhuazuopin',that.detail)
							setTimeout(()=>{
